...
from navbev.perception.backend.utils.utils import compute_centroid
...
class Loss:
    ...
    def __call__(self, inputs, targets):
        ...
        if "centroid" in self.args.loss:
            ...
            gt_cent_x, gt_cent_y = (
                gt_centroid / self.mask_dim,
                gt_centroid % self.mask_dim,
            )
            gt_cent_x, gt_cent_y = gt_cent_x / self.mask_dim, gt_cent_y / self.mask_dim

            loss += self.mse_loss(cent_x, gt_cent_x) + self.mse_loss(cent_y, gt_cent_y)
        if loss == 0:
